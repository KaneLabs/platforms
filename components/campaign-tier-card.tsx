...
export default function CampaignTierCard({ tier, currency, onClickEdit, onClickDelete, isSelected, onClickSelect }:
  ...
  return (
    <div
      className={`flex flex-col space-y-4 my-4 rounded-lg border py-6 px-8 bg-gray-100 dark:bg-gray-800 transition ease-in-out ${onClickSelect && isSelected ? 'border border-accent-green bg-white dark:bg-gray-800' : 'border border-gray-300'} ${onClickSelect && 'cursor-pointer hover:bg-white hover:dark:bg-gray-800'} `}
      onClick={() => {
        if (onClickSelect) {
